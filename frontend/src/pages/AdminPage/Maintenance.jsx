    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          #maintenance-print-area, #maintenance-print-area * { visibility: visible; }
          #maintenance-print-area { position: absolute; left: 0; top: 0; width: 100%; font-family: Arial, sans-serif; font-size: 11pt; }
      ...
      {/* ── HIDDEN PRINT AREA ── */}
      <div id="maintenance-print-area" className="hidden print:block">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder="Search unit, tenant, title, or category..."
                value={search}
                        ...
                        <td className="px-5 py-4 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              ...
                              className="text-[11px] font-bold text-red-500 hover:text-red-700 transition-colors uppercase tracking-widest"
                            >
                              Delete
                            </button>
                          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl border border-slate-100 overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Requested By</p>
                  <p className="text-base font-bold text-slate-900">{viewModal.tenant?.fullName}</p>
                  <p className="text-xs text-slate-500 mt-0.5">Unit {viewModal.tenant?.unitNumber}</p>
                </div>
                <div className="sm:text-right">
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Current Status</p>
                  <span className={`inline-block text-[10px] font-bold px-3 py-1 rounded-md border uppercase tracking-widest ${STATUS_CONFIG[viewModal.status]?.color || "bg-slate-50 text-slate-600 border-slate-200"}`}>
                    {viewModal.status}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Issue</p>
                  <p className="text-sm text-slate-800">{viewModal.title}</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Category</p>
              ...
              <div className="grid grid-cols-3 gap-4 pt-4 border-t border-slate-100">
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Requested</p>
                  <p className="text-xs text-slate-800">{fmt(viewModal.requestedDate)}</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">Start Date</p>
                  <p className="text-xs text-slate-800">{fmt(viewModal.startDate)}</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">End Date</p>
                  <p className="text-xs text-slate-800">{fmt(viewModal.endDate)}</p>
                </div>
              </div>
            </div>
          </div>
